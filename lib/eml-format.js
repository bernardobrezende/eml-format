  ...
  function _read(data) {
    try {
      ...
      if (data.headers["Subject"]) {
        var subjectHeader = data.headers["Subject"]
        var parts = /=\?UTF\-8\?(B|Q)\?(.+?)(\?=)/gi.exec(subjectHeader)
        if (parts && parts.length) {
          const [ , encodingType, subjectValue ] = parts
          if (encodingType === 'B') {
            result.subject = emlformat.unquoteUTF8(subjectValue)
          } else if (encodingType === 'Q') {
            result.subject = emlformat.unquoteString(_decodeText(subjectValue))
          } 
        } else {
