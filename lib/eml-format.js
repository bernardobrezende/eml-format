...
const _decodeText = (text, charset) => {
  try {
      return utf8.decode(quotedPrintable.decode(text))
  } catch (err) {
      console.log(err)
      return emlformat.unquotePrintable(text, charset)
  }
}

const REGEX = {
  UTF8: new RegExp(/=\?UTF\-8\?(B|Q)\?(.+?)(\?=)/gi)
}
  ...
  function _read(data) {
    try {
      ...
      if (data.headers["Subject"]) {        
        // this is needed by cases where the subject contains more than one encoding
        // e.g: "=?utf-8?B?firstPartOfSubjectInBase64?=\r\n=?utf-8?Q?lastPartOfSubjectInQuotePrintable?="
        const subjectLineBreaks = data.headers["Subject"].split(/\r\n/g);
        result.subject = subjectLineBreaks.map(subject => {
          var parts = /\=\?(:?UTF-8|ISO-8859-1)?\?(:?B|Q)?\?(:?.+)?\?\=/gi.exec(subject)
          if (Array.isArray(parts)) {
            const [ , subjectCharset, encodingType, subjectValue ] = parts
            if (encodingType === 'B') {
              return Buffer.from(subjectValue.replace(/\r?\n/g, ""), "base64").toString("utf8");
            } else if (encodingType === 'Q') {
              if (subjectCharset === 'utf-8') {
                return emlformat.unquoteString(_decodeText(subjectValue))
              }
              return emlformat.unquotePrintable(subjectValue, subjectCharset);
            } 
          }
        }).join('') || emlformat.unquoteString(data.headers["Subject"]);
      }
      ...
      function _append(headers, content) {
        ...
        if (encoding == "base64") {
          ...
          content = _decodeText(content, charset)
        }
...
function parseRecursive(lines, start, parent, options) {
  ...
  for (var i = start; i < lines.length; i++) {
    ...
    if (!insideBody) {
      ...
      // fallback regex added to support optional value and space (between : and value)
      var match = /^([\w\d\-]+):\s+([^\r\n]+)/gi.exec(line) || /^([\w\d\-]+):(\s+)?([^\r\n]+)?/gi.exec(line);
      if (match) {
